...
class Installer:
    ...
    def run(self) -> ToolModel | None:
        ...
        if lookup_name not in asset.name.lower():
            if not click.confirm(f'Asset "{asset.name}" doesn\'t match "{self._name or self.repo}". Install anyway?', default=False):
                raise click.Abort()
        ...
        if self._rename_to is not None:
            ...
            rename_to = click.prompt(f'Rename binary "{original_name}"? (leave blank to keep as-is)', default='', show_default=False).strip() or None
        else:
            rename_to = None
